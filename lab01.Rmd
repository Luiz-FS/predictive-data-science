...
grafico <- dados %>% ggplot(aes(x=reorder(nomeParlamentar, total), y=total)) + 
  geom_col(width = 0.7, show.legend = FALSE) + 
  coord_flip() + 
  labs(x="Nome do parlamentar", y="Total gasto")

# Adicionando informações sobre os limites de cada deputado em seu estado
grafico + geom_col(aes(x=reorder(nomeParlamentar, limite_mensal), y=limite_mensal, fill=nomeParlamentar), width = 0.7, show.legend = FALSE)
```
